...
int main(void){
	...
	DIO_void_set_pin_dir(DIO_REF_PORTD, B0, OUTPUT);
	DIO_void_set_pin_dir(DIO_REF_PORTD, B1, OUTPUT);
	DIO_void_set_pin_dir(DIO_REF_PORTD, B2, OUTPUT);
	DIO_void_set_pin(DIO_REF_PORTD, B0);
	DIO_void_set_pin(DIO_REF_PORTD, B1);
	DIO_void_set_pin(DIO_REF_PORTD, B2);
	DIO_void_set_pin_dir(DIO_REF_PORTD, B3, INPUT);
	DIO_void_set_pin_dir(DIO_REF_PORTD, B4, INPUT);
	DIO_void_set_pin_dir(DIO_REF_PORTD, B5, INPUT);
	...
	while(1){
		for(u8 COL= 0; COL < 3; COL++){
			//select one column
			DIO_void_set_pin(DIO_REF_PORTD, COL);
			//check which row is pressed
			for(u8 ROW= 3; ROW < 6; ROW++){
				//check if current row is pressed
				if(HIGH == DIO_u8_get_pin(DIO_REF_PORTD, ROW)){
					u8 value_id= ((COL)+(3*ROW)-8);
					DIO_void_assign_port(DIO_REF_PORTC, seg_val_dec[value_id]);
					_delay_ms(50);
				}
			}
			DIO_void_clear_pin(DIO_REF_PORTD, COL);
		}
	}
	return 0;
}
